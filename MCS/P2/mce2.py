...
STOCK_SIMULATIONS = 100000          # Number of Monte Carlo runs
STOCK_INITIAL_PRICE = 100          # Starting stock price ($)
STOCK_MU = 0.08                    # Expected annual return (10%)
STOCK_SIGMA = 0.20                 # Volatility/uncertainty (20%)
STOCK_DT = 1/252                   # Time step (1 day, 252 trading days/year)
STOCK_DAYS = 252                   # Time horizon (252 trading days = 1 year)

# --- PORTFOLIO SIMULATION ---
PORTFOLIO_SIMULATIONS = 50000       # Number of Monte Carlo runs
PORTFOLIO_INITIAL = 100000         # Starting investment ($100,000)
PORTFOLIO_ASSET_RETURNS = [0.10, 0.04]      # Expected returns: [Stocks 10%, Bonds 4%]
PORTFOLIO_WEIGHTS = [0.70, 0.30]            # Portfolio allocation: [70% stocks, 30% bonds]
PORTFOLIO_YEARS = 40               # Time horizon (10 years)
PORTFOLIO_NOISE_STD = 0.15         # Volatility of returns (15% standard deviation)
...
simulator = MonteCarloSimulator(n_simulations=STOCK_SIMULATIONS)
...
results = simulator.simulate_paths(
    initial_value=STOCK_INITIAL_PRICE,
    n_steps=STOCK_DAYS,
...
# Extra insights
print(f"Probability stock doubles: {results.probability_below(STOCK_INITIAL_PRICE * 2).__sub__(1):.2%}")
print(f"Probability stock loses 20%: {results.probability_below(STOCK_INITIAL_PRICE * 0.8):.2%}")
...
class PortfolioSimulator(MonteCarloSimulator):
    ...
    def simulate_portfolio(self, 
        ...
        def step_fn(values, step):
            ...
            noise = np.random.normal(weighted_return, PORTFOLIO_NOISE_STD, size=len(values))
...
# Run portfolio simulation with parameters from top
portfolio_sim = PortfolioSimulator(n_simulations=PORTFOLIO_SIMULATIONS)
portfolio_results = portfolio_sim.simulate_portfolio(
    initial_investment=PORTFOLIO_INITIAL,
    asset_returns=PORTFOLIO_ASSET_RETURNS,
    asset_weights=PORTFOLIO_WEIGHTS,
    n_years=PORTFOLIO_YEARS
)

portfolio_sim.print_results(portfolio_results)
print(f"After {PORTFOLIO_YEARS} years, likely to have: ${portfolio_results.stats['median']:,.0f}")
